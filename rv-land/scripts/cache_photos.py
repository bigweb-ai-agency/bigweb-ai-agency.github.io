...
def fetch_photo(row):
    ...
    if current.startswith('assets/listings/') and (ROOT/current).is_file():return x,'cached'
    url=clean_image_url(current)
    if not url:return x,'missing'
    try:
        if url.startswith('data:image/'):
            header,payload=url.split(',',1);mime=header[5:].split(';')[0];content=base64.b64decode(payload)
        else:
            if urlparse(url).scheme!='https' or urlparse(url).hostname not in HOSTS:return x,'unsupported'
            with requests.get(url,timeout=20,stream=True) as response:
                response.raise_for_status()
                if urlparse(response.url).hostname not in HOSTS:return x,'unsupported_redirect'
                mime=response.headers.get('Content-Type','').split(';')[0]
                if mime not in MIME:return x,'not_image'
                parts=[];size=0
                for part in response.iter_content(65536):
                    size+=len(part)
                    if size>5_000_000:return x,'too_large'
                    parts.append(part)
                content=b''.join(parts)
            time.sleep(.2)
        if mime not in MIME or len(content)<100:return x,'not_image'
        ...
        if not any(signatures):return x,'invalid_image'
        ...
        x['image_url']=rel;x['photo_preserved_at']=NOW
        ...
        x['image_url']=url;x['photo_error']='Network / source unavailable'
